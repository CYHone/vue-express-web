...
const email = ref('');
const shipmentInfo = ref(''); // 初始化为空对象
  ...
  axios.get(`shipment/getShipmentInfo?id=${shipmentId.value}`)
    .then(response => {
      ...
      if (response.data.code === 200) {
        ...
        // shipmentInfo.value = data; // 将获取到的运单信息存储到 ref 对象中
        // ElMessage.success('成功获取运单信息');
        // console.log(data); // 输出运单信息到控制台
        // console.log(data.id); // 输出运单ID到控制台


        console.log(shipmentInfo); // 输出运单信息到控制台
        console.log(shipmentInfo.id); // 输出运单ID到控制台

        //showShipmentInfoCard(shipmentInfo);
  ...
  <div class="order">
    ...
    <el-form-item label="获取运单和附属包裹信息" prop="shipmentId">
      ...
      <el-card style="height: 400px; width: 600px;">
        ...
        <div>
          < <div>
            <p>包裹ID: {{ shipmentInfo.id }}</p>
            <p>创建日期: {{ shipmentInfo.createDate }}</p>
            <p>始发地: {{ shipmentInfo.origin }}</p>
            <p>目的地: {{ shipmentInfo.destination }}</p>
            <p>价格: {{ shipmentInfo.price }}</p>
            <p>状态: {{ shipmentInfo.status }}</p>
            <p>顾客ID: {{ shipmentInfo.customerId }}</p>
            <p>类型: {{ shipmentInfo.type }}</p>
          </div>
        </div>
      </el-card>
